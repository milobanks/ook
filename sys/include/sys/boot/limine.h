...
#ifndef OOK_LIBKERN_LIMINE_LIMINE_H
#define OOK_LIBKERN_LIMINE_LIMINE_H

#include <cstdint>

namespace ook::limine {
/* Misc */
...
#define LIMINE_REQUESTS_START_MARKER \
    uint64_t requests_start_marker[4] = { \
        0xf6b8f4b39de7d1ae, \
        0xfab91a6940fcb9cf, \
        0x785c6ed015d3e316, \
        0x181e920a7852b9d9 \
...
#define LIMINE_REQUESTS_END_MARKER \
    uint64_t requests_end_marker[2] = {0xadc0e0531bb10d03, 0x9572709f31764c62};

#define LIMINE_REQUESTS_DELIMITER LIMINE_REQUESTS_END_MARKER

#define LIMINE_BASE_REVISION(N) \
    uint64_t base_revision[3] = {0xf9562b2d5c95a6c8, 0x6a7b384944536bdc, (N)};

#define LIMINE_BASE_REVISION_SUPPORTED (base_revision[2] == 0)

#define LIMINE_LOADED_BASE_REV_VALID (base_revision[1] != 0x6a7b384944536bdc)
#define LIMINE_LOADED_BASE_REVISION (base_revision[1])
...
#define LIMINE_BOOTLOADER_INFO_REQUEST \
    ...
    struct bootloader_info_request {
        uint64_t id[4];
        uint64_t revision;
        LIMINE_PTR(struct bootloader_info_response*) response;
    };
...
#define LIMINE_FIRMWARE_TYPE_SBI 3
    ...
    struct firmware_type_request {
        uint64_t id[4];
        uint64_t revision;
        LIMINE_PTR(struct firmware_type_response*) response;
    };
...
#define LIMINE_STACK_SIZE_REQUEST \
    ...
    struct stack_size_request {
        uint64_t id[4];
        uint64_t revision;
        LIMINE_PTR(struct stack_size_response*) response;
        uint64_t stack_size;
    };
...
#define LIMINE_HHDM_REQUEST \
    ...
    struct hhdm_request {
        uint64_t id[4];
        uint64_t revision;
        LIMINE_PTR(struct hhdm_response*) response;
    };
...
#define LIMINE_FRAMEBUFFER_RGB 1
    ...
    struct framebuffer {
        ...
        /* Response revision 1 */
        uint64_t mode_count;
        LIMINE_PTR(struct video_mode**) modes;
    };

    struct framebuffer_response {
        uint64_t revision;
        uint64_t framebuffer_count;
        LIMINE_PTR(struct framebuffer**) framebuffers;
    };

    struct framebuffer_request {
        uint64_t id[4];
        uint64_t revision;
        LIMINE_PTR(struct framebuffer_response*) response;
    };
...
#define LIMINE_TERMINAL_OOB_OUTPUT_OPOST (1 << 7)
    ...
    struct LIMINE_DEPRECATED terminal;

    typedef void (*terminal_write)(struct terminal*, const char*, uint64_t);
    typedef void (*terminal_callback)(
        struct terminal*,
        uint64_t,
        uint64_t,
        uint64_t,
    ...
    struct LIMINE_DEPRECATED terminal {
        uint64_t columns;
        uint64_t rows;
        LIMINE_PTR(struct framebuffer*) framebuffer;
    };

    struct LIMINE_DEPRECATED terminal_response {
        uint64_t revision;
        uint64_t terminal_count;
        LIMINE_PTR(struct terminal**) terminals;
        LIMINE_PTR(terminal_write) write;
    };

    struct LIMINE_DEPRECATED terminal_request {
        uint64_t id[4];
        uint64_t revision;
        LIMINE_PTR(struct terminal_response*) response;
        LIMINE_PTR(terminal_callback) callback;
    };
...
#define LIMINE_PAGING_MODE_REQUEST \
    ...
    struct paging_mode_request {
        uint64_t id[4];
        uint64_t revision;
        LIMINE_PTR(struct paging_mode_response*) response;
        uint64_t mode;
        uint64_t max_mode;
        uint64_t min_mode;
    };
...
#define LIMINE_5_LEVEL_PAGING_REQUEST \
    ...
    struct LIMINE_DEPRECATED five_level_paging_response {
        uint64_t revision;
    };

    struct LIMINE_DEPRECATED five_level_paging_request {
        uint64_t id[4];
        uint64_t revision;
        LIMINE_PTR(struct five_level_paging_response*) response;
    };
    ...
    #define LIMINE_MP(TEXT) mp_##TEXT
#else
    #define LIMINE_SMP_REQUEST \
        {LIMINE_COMMON_MAGIC, 0x95a67b819a1b857e, 0xa0b61b723b6a73e0}
    #define LIMINE_MP(TEXT) smp_##TEXT
#endif

    struct LIMINE_MP(info);

    typedef void (*goto_address)(struct LIMINE_MP(info) *);
    ...
    struct LIMINE_MP(info) {
        ...
        uint32_t lapic_id;
        uint64_t reserved;
        LIMINE_PTR(goto_address) address;
...
#define LIMINE_MEMMAP_FRAMEBUFFER 7
    ...
    struct memmap_response {
        ...
        LIMINE_PTR(struct memmap_entry**) entries;
    };

    struct memmap_request {
        uint64_t id[4];
        uint64_t revision;
        LIMINE_PTR(struct memmap_response*) response;
    };
...
#define LIMINE_ENTRY_POINT_REQUEST \
    ...
    struct entry_point_request {
        uint64_t id[4];
        uint64_t revision;
        LIMINE_PTR(struct entry_point_response*) response;
        LIMINE_PTR(entry_point) entry;
    };
    ...
    struct kernel_file_response {
        ...
        uint64_t revision;
#if LIMINE_API_REVISION >= 2
        LIMINE_PTR(struct file*) executable_file;
#else
        LIMINE_PTR(struct file*) kernel_file;
#endif
    };
...
#else
    struct kernel_file_request {
#endif
        uint64_t id[4];
        uint64_t revision;
#if LIMINE_API_REVISION >= 2
        LIMINE_PTR(struct executable_file_response*) response;
#else
        LIMINE_PTR(struct kernel_file_response*) response;
#endif
    };
...
#define LIMINE_INTERNAL_MODULE_COMPRESSED (1 << 1)
    ...
    struct module_response {
        ...
        LIMINE_PTR(struct file**) modules;
    };

    struct module_request {
        uint64_t id[4];
        uint64_t revision;
        LIMINE_PTR(struct module_response*) response;

        /* Request revision 1 */
        uint64_t internal_module_count;
        LIMINE_PTR(struct internal_module**) internal_modules;
    };
...
#define LIMINE_RSDP_REQUEST \
    ...
    struct rsdp_request {
        uint64_t id[4];
        uint64_t revision;
        LIMINE_PTR(struct rsdp_response*) response;
    };
...
#define LIMINE_SMBIOS_REQUEST \
    ...
    struct smbios_request {
        uint64_t id[4];
        uint64_t revision;
        LIMINE_PTR(struct smbios_response*) response;
    };
...
#define LIMINE_EFI_SYSTEM_TABLE_REQUEST \
    ...
    struct efi_system_table_request {
        uint64_t id[4];
        uint64_t revision;
        LIMINE_PTR(struct efi_system_table_response*) response;
    };
...
#define LIMINE_EFI_MEMMAP_REQUEST \
    ...
    struct efi_memmap_request {
        uint64_t id[4];
        uint64_t revision;
        LIMINE_PTR(struct efi_memmap_response*) response;
    };
...
#define LIMINE_BOOT_TIME_REQUEST \
    ...
    struct boot_time_request {
        uint64_t id[4];
        uint64_t revision;
        LIMINE_PTR(struct boot_time_response*) response;
    };
...
#if LIMINE_API_REVISION >= 2
    struct executable_address_response {
#else
...
#if LIMINE_API_REVISION >= 2
    struct executable_address_request {
#else
    struct kernel_address_request {
#endif
        uint64_t id[4];
        uint64_t revision;
#if LIMINE_API_REVISION >= 2
        LIMINE_PTR(struct executable_address_response*) response;
#else
        LIMINE_PTR(struct kernel_address_response*) response;
#endif
    };
...
#define LIMINE_DTB_REQUEST \
    ...
    struct dtb_request {
        uint64_t id[4];
        uint64_t revision;
        LIMINE_PTR(struct dtb_response*) response;
    };
...
#define LIMINE_RISCV_BSP_HARTID_REQUEST \
    ...
    struct riscv_bsp_hartid_request {
        uint64_t id[4];
        uint64_t revision;
        LIMINE_PTR(struct riscv_bsp_hartid_response*) response;
    };
